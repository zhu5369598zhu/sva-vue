<template>
  <div class="mod-speed" style="height: 100%;">
    ...
    <div class="tabs">
      ...
      <el-tab-pane label="图表" name="chart" actived="true">
        <div v-show="hasData===true&&chartType==='chartbar'" id="chartbar" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartbar" :unNormal="unNormal" :normal="normal" :all="all" :category="category" :legend="legend" :series="series" title="速度数据图表" @dblclick="chartDblClick"></component>
        </div>
        <div v-show="hasData===true&&chartType==='chartline'" id="chartline" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartline" :unNormal="unNormal" :normal="normal" :all="all" :category="category" :legend="legend" :series="series" title="速度数据图表" @dblclick="chartDblClick"></component>
        </div>
        <div v-show="hasData===true&&chartType==='chartpie'" id="chartpie" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartpie" :unNormal="unNormal" :normal="normal" :all="all" :category="category" :legend="legend" :series="series" title="速度数据图表" @dblclick="chartDblClick"></component>
        </div>
    ...
    methods: {
      ...
      view (type, url) {
        if (type !== 'data') {
          ...
          this.$nextTick(() => {
            this.$refs.viewChart.init('speed', url)
          })
        }
      },
      changeImg (type) {
      ...
      chartDblClick (index) {
        ...
        this.$nextTick(() => {
          this.$refs.viewData.init(id, 'speed')
        })
      },
      getChartData () {
        this.legend = []
