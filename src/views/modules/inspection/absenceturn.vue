    ...
    <div class="show-data-table">
      ...
      <el-tab-pane label="图表" name="chart" actived="true">
        <div v-show="hasData===true&&chartType==='chartbar'" id="chartbar" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartbar" :category="category" :legend="legend" :inspect="inspect" :inspected="inspected" title="缺勤统计图表"></component>
        </div>
        <div v-show="hasData===true&&chartType==='chartpie'" id="chartpie" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartpie" :legend="legend" :series="series" title="缺勤周期图表"></component>
        </div>
...
<script>
  ...
  import chartbar from '@/components/charts/bar3'
