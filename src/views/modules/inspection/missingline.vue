    ...
    <div class="show-data-table">
      ...
      <el-tab-pane label="图表" name="chart" actived="true">
        ...
        <div v-show="hasData===true&&chartType==='chartpie'" id="chartpie" :style="{ 'height': chartHeight + 'px' }">
          <component  :is="chartType" ref="chartpie" :legend="legend" :series="series" title="漏检率统计图表"></component>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/inspection/inspectiontaskdevice/getlinebydate'),
          method: 'get',
      ...
      getStatisticsByDate () {
        ...
        this.$http({
          url: this.$http.adornUrl('/inspection/inspectiontaskdevice/getlinestatisticsbydate'),
          method: 'get',
          params: this.$http.adornParams({
            'deptId': this.dataForm.deptId,
            'lineId': this.dataForm.lineId,
      ...
      exportToExcel (list) {
        ...
        require.ensure([], () => {
          ...
          const tHeader = ['巡线名称', '所属机构', '应检巡点数', '已检巡点数', '漏检巡点数', '巡点漏检率', '应检巡项数', '已检巡项数', '漏检巡项数', '巡项漏检率']
          const filterVal = ['lineName', 'deptName', 'inspectDeviceCount', 'inspectedDeviceCount', 'inspectMissDeviceCount', 'inspectMissDeviceRate','inspectItemCount','inspectedItemCount', 'inspectMissItemCount','inspectMissItemRate']
