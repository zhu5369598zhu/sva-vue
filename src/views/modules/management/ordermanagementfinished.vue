    ...
    methods: {
      ...
      clickRow (row) {
        this.$http({
          ...
          if (this.orderDataForm.orderStatus === 4) {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/management/ordermanagementfinished/list'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            ...
            'orderApplicantId': this.loginuserId,
            'startTime': startTime,
            'endTime': endTime
          })
        }).then(({data}) => {
